...
# Description:  This is the solution for course Project 2 (plot 4) for
...
run_analysis <- function(){
  ...
  coal.matches <<- SCC[grepl("coal", SCC$Short.Name, ignore.case=TRUE),]
  
  #Get the data
  coal.matches.data <<- merge(x=NEI, y=coal.matches, by="SCC")
  
  #Calculate the emissions
  total_emissions_by_year <<- aggregate(coal.matches.data[, "Emissions"], by=list(coal.matches.data$year), sum)
  ...
  ggp <- ggplot(data=total_emissions_by_year, aes(x=year, y=Emissions/10^6)) + 
    geom_line(aes(group=1, col=Emissions)) + 
    geom_point(aes(size=2, col=Emissions)) + 
    ggtitle("Total PM2.5 Emissions") + 
    ylab("PM2.5 Emissions (Tons)") + 
    geom_text(aes(label=round(Emissions/10^6,digits=2), size=2, hjust=1.5, vjust=1.5)) + 
